...
export default function SaleInvoicePreview({
  ...
  /* A5 prints landscape, A4 portrait; the heading claimed Landscape for
     both. Kept as a note under the title rather than inside it, so the
     title stays the same length whatever paper the shop has chosen. */
  const orientation = isA5 ? "Landscape" : isThermal ? "Roll" : "Portrait";

  return (
    <PrintPreviewModal
      title="پیش‌نمایش فاکتور فروش"
      onPrint={handlePrint}
      onClose={onClose}
      ...
      note={
        <>
          <p className="mt-1 text-body-xs text-text-secondary">
            کاغذ: {paperSize} — {orientation}
          </p>
          <p className="mt-2 text-body-xs text-warning-fg">
    ...
    >
      ...
      >
        ...
        <div className={`grid grid-cols-2 gap-3 ${spacing.section}`}>
          {/* Seller Info */}
          <div className={`border border-border rounded-lg ${spacing.padding}`}>
            <h3
              className={`font-medium text-text-primary mb-1 pb-1 border-b border-border ${fontSize.heading}`}
            >
              مشخصات فروشنده
            </h3>
            <div className={`space-y-0.5 ${fontSize.small} text-text-primary`}>
              ...
              {settings?.company_phone && (
                <p>
                  ...
                  {toPersianDigits(settings.company_phone)}
                </p>
              )}
          ...
          <div className={`border border-border rounded-lg ${spacing.padding}`}>
            ...
            <div className={`space-y-0.5 ${fontSize.small} text-text-primary`}>
              ...
              {showCustomerPhone && (
                <p>
                  <span className="font-bold"> تلفن : </span>
                  {toPersianDigits(invoice.customer_phone)}
                </p>
              )}
            </div>
          </div>
            ...
            <div className="flex items-start gap-4">
              {showSignature && settings?.signature_image && (
                <div className="text-center">
                  ...
                  />
                  <p className={`text-text-secondary mt-0.5 ${fontSize.small}`}>
                    امضا
                  </p>
                </div>
              )}
