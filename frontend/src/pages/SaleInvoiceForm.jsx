...
// Modal برای تعریف سریع کالا (دقیقاً مثل PurchaseInvoiceForm)
...
export default function SaleInvoiceForm() {
    ...
    setSelectedItems((prev) =>
      prev.map((item, i) => {
        ...
        if (field === "item_id" && value) {
          ...
          if (selectedItem) {
            // پیشنهاد قیمت فروش: میانگین قیمت خرید + ۲۰٪ سود
            const suggestedPrice = Math.round(
              (selectedItem.avgPurchasePrice || 0) * 1.2,
            );
            updated.unit_price = suggestedPrice;
          }
        }
  ...
  const handleQuickItemSuccess = (newItem) => {
    fetchData(); // رفرش لیست کالاها
    toast.success(
      `کالای "${newItem.name}" اضافه شد. حالا می‌توانید آن را انتخاب کنید.`,
    );
  };
                ...
                <div className="space-y-4">
                  {selectedItems.map((item, index) => {
                    ...
                    return (
                      ...
                      >
                        <div className="grid grid-cols-12 gap-5 items-end py-1">
                          <div className="col-span-5 relative">
                            ...
                            />
                            {selectedItem && (
                              <p className="mt-1 text-xs text-gray-500 absolute top-0 left-0">
