...
| تعداد policy  | **۳۳** — شمارش دقیق پایین |
| توابع `app_*` | **۴ تا**                  |
...
⚠️ عدد ۳۳ حاصل جمع ساده‌ی «جدول‌های `workspace_id` دار + ۱» نیست، و قبلاً بود.
سه استثنا آن را شکستند: `otp_codes` که `workspace_id` ندارد ولی policy دارد،
`referrals` که به‌جای یک ستون دو ستون کارگاه دارد، و `referral_codes` که دو
policy دارد (خواندن باز، نوشتن محدود).

چهار policy آخر از ۱۲.۱ می‌آیند: `sms_wallets`، `sms_wallet_transactions`،
...
و با همین کوئری روی دیتابیس سالم مقایسه کن. `smoke.test.ts` همین را
خودکار چک می‌کند.
